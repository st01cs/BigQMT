...
> 状态：设计草案（未编码）
> 定位：BigQMT 仅支持原生 QMT（完整版客户端），参考 EasyXT 的连接管理设计，但去除 miniQMT 与信号桥接分支。
...
- EasyXT 以 miniQMT 为主、大 QMT 走信号桥接；BigQMT **直接用原生客户端 + xtquant API**，不做桥接，也没有 miniQMT 分支。
- EasyXT 里「大 QMT 已运行」≈ 跳过 miniQMT 登录；BigQMT 里「原生 QMT 已运行」≠ 可用，还必须 **已登录 + xtdata/交易通道连通** 才算就绪。
...
| 参考模块 | 职责 |
| --- | --- |
| `core/qmt_connection.py` | 连接管理器：单例 + 状态检查 + 便捷函数 + 装饰器 |
| `core/auto_login/qmt_login.py` | pywinauto 自动登录、进程/登录态检测 |
| `easy_xt/qmt_paths.py` | 安装目录 / userdata 路径发现 |
| `easy_xt/start_qmt.py` / `start_qmt_interactive.py` | 命令行入口 |

> 注：EasyXT 中不存在 `core\qmt\_connection.py`，对应主体实际在 `core/qmt_connection.py`。
...
│     ├─ _paths.py            # 安装目录/userdata 发现（参考 qmt_paths.py）
│     ├─ _process.py          # 进程/窗口/登录态检测（tasklist + pywinauto）
│     ├─ _auto_login.py       # 原生客户端自动登录（参考 qmt_login.py）
...
若希望与 EasyXT 目录结构保持一致，也可把模块放在仓库根 `core/qmt/`，内部设计不变。

## 4. 状态机（核心）

QmtManager 用显式状态机管理生命周期，避免 EasyXT 中「30 秒内直接返回 True」的假阳性缓存：
...
### _connection.py（QmtManager，参考 core/qmt_connection.py）
...
- 启动早期对 xtdata 下载方法做全局加锁（参考 EasyXT `_patch_xtdata_download`），防并发下载卡死。

### cli.py

- `python -m bigqmt.core.qmt status|login|start|stop|restart`，退出码 0/非 0 便于脚本判断。

## 6. 与 EasyXT 的关键差异（为何重写而非照搬）

1. EasyXT 连接管理器 `ensure_qmt_logged_in` 的 30 秒缓存是「直接返回 True」而未复核状态——BigQMT 只在心跳确认健康后才缓存。
2. EasyXT 面向 miniQMT 自动登录，原生 QMT 走桥接；BigQMT 要直接对原生客户端做 GUI 登录 + xtquant 直连，登录健壮性要求更高（含人工兜底）。
3. 原生客户端登录界面更重（可能含服务器选择、验证码、行情/交易双登录），登录态判定须多探针组合而非单一窗口标题。
4. BigQMT 把「已登录可交易」收敛为一个 READY 态，供策略/数据层统一依赖，而不是各自再查一遍。

## 7. 分阶段实施（含测试，遵循 AGENTS.md）
