...
- 复用 EasyXT 的两点（不照搬）：
  - `live_strategies/*/start.py`：策略 = 独立 Python 子进程；
  - `multi_strategy_manager.py`：PID 落盘、日志、崩溃重启上限。
  - 去掉其调度器 / 多策略冲突检测等重框架，只做「单条策略 + 生命周期随 QMT」。
- 不做：
...
| 形态 | 说明 | 判定 |
| --- | --- | --- |
| A. 外部 Python 策略进程（推荐/默认） | 用户给一段可执行命令（如 `python D:\strat\main.py`），READY 后由 BigQMT 以子进程拉起，注入 QMT 环境变量 | 与现有架构、EasyXT 实盘策略一致；可测试、可保活 |
